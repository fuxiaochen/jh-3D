                ...
                <div class="noticeDepart">
                    <div class="noticeDepartItem">
                        <a-form-model-item label="时间" prop="departmentDate">
                            <a-date-picker :valueFormat="valueFormat" show-time @change="departmentDate" v-model="baseData.departmentDate"  :disabled="isDisabled"/>
                        </a-form-model-item>
                    </div>
                </div>

                <p><b>7.2部门ERT集合广播</b></p>
                <div class="noticeDepart">
                    <div class="noticeDepartItem">
...
import { log } from 'three';
    export default {
        ...
        methods: {
            ...
            // onSelectChange6(selectedRowKeys) {
            //     this.selectedRowKeys6 = selectedRowKeys;
            //     console.log('this.selectedRowKeys6',this.selectedRowKeys6);
            // },
            ...
            submitForm(formName) {
                this.$refs[formName].validate(valid => {
                    if (valid) {
                        ...
                        this.$http.post3('/chemDeptErt/executeProcess', data).then(res => {
                            if (res.data.code === 1) {
                                let data = {
                                    processInstanceId:this.processInstanceId
                                }
            ...
            saveData() {
                ...
                this.$http.post3('/chemDeptErt/saveOrUpdate', data).then(res => {
                    this.$message("success", "保存成功");
                })
            },
            viewData() {
                this.$http.get3('/chemDeptErt/queryOne?processInstanceId=' + this.processInstanceId).then(res => {
                    if (res.data.data){
