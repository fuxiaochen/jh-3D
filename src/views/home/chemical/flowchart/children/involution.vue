<template>
	<div id="involution">
		...
		<template>
			...
			>
			<a-form-model-item>
				<p style="margin-left: -40px"><b>12.1<span class="star">*</span>现场环境侦测（多次）</b></p>
				...
				<a-form-model-item prop="TVdate" >
					<div style="margin-left: -40px"><span><b>12.2<span class="star">*</span>现场环境清理，完成时间：</b></span><a-date-picker :valueFormat="valueFormat" show-time v-model="dynamicValidateForm.TVdate"  :disabled="isDisabled"/></div>
				</a-form-model-item>
					<a-form-model-item prop="TVdate" >
					<div style="margin-left: -40px"><span><b>12.3<span class="star">*</span>器材复归，完成时间：</b></span><a-date-picker :valueFormat="valueFormat" show-time v-model="dynamicValidateForm.equipmentDetectionTime" :disabled="isDisabled"/></div>
				</a-form-model-item>
					<a-form-model-item prop="TVdate" >
					<div style="margin-left: -40px"><span><b>12.4<span class="star">*</span>管制解除，完成时间：</b></span><a-date-picker :valueFormat="valueFormat" show-time v-model="dynamicValidateForm.deregulationTime" :disabled="isDisabled"/></div>
				</a-form-model-item>
				<a-form-model-item>
				<p style="margin-left: -40px"><b>12.5<span class="star">*</span>回报指挥官，解除状况或人员复归确认</b></p>
				<div class="add-new-user">
					<span style="margin-left: -20px;width:80px"><b>指挥官：</b></span><div class="right"><a-input allowClear v-model="dynamicValidateForm.commander" placeholder="请输入总指挥官" :disabled="isDisabled" /></div>
					<span style="margin-left: 20px;width:60px"><b>分机：</b></span><div class="right"><a-input allowClear v-model="dynamicValidateForm.ext" placeholder="请输入分机" :disabled="isDisabled" /></div>
					<span style="margin-left: 20px;width:60px"><b>时间：</b></span><div class="right"><a-date-picker :valueFormat="valueFormat" show-time v-model="dynamicValidateForm.reportTime" :disabled="isDisabled" /> </div>
				</div>
				</a-form-model-item>
				<a-form-model-item prop="TVdate" >
					<div style="margin-left: -40px"><span><b>12.6<span class="star">*</span>发布复归广播，时间：</b></span><a-date-picker :valueFormat="valueFormat" show-time v-model="dynamicValidateForm.radioBroadcastTime" :disabled="isDisabled" /></div>
					<div>  复归广播词：监控中心报告，监控中心报告，先前(☆☆)发生火灾，现已处理完毕，现场环境确认无异常，请同仁返回岗位安心工作。（间隔1.5S，重复两次）</div>
				</a-form-model-item>
				<a-form-model-item prop="TVdate" >
					
					<div style="margin-left: -40px"><span><b>12.7<span class="star">*</span>发送复归简讯（前提是有人员疏散/发送初期应变失败简讯），时间：</b><a-date-picker :valueFormat="valueFormat" show-time v-model="dynamicValidateForm.newsTime" :disabled="isDisabled" /></span>
						...
						<a-form-model-item>
							<a-radio-group v-model="dynamicValidateForm.region" class="fire-radio-group" name="radioGroup" @change="seeFire">
								<a-radio :value="1">
									...
									<div v-if="dynamicValidateForm.region === 1" style="margin-top:10px"><span><b>通报时间：</b></span><a-date-picker :valueFormat="valueFormat" show-time v-model="dynamicValidateForm.accidentNotificationTime" :disabled="isDisabled" /><span style='margin-left:20px'><b>通报人：</b></span><a-input v-model="inputPople" :disabled="isDisabled" /></div> 
								</a-radio>
								<a-radio :value="2">
									污染物异常大量泄漏或其他造成污染环境或危害人体健康之情况，环保课须于一小时内向新站区管委会环保局进行通报
									<div v-if="dynamicValidateForm.region === 2" style="margin-top:10px"><span><b>通报时间：</b></span><a-date-picker :valueFormat="valueFormat" show-time v-model="dynamicValidateForm.pollutantNotificationTime" :disabled="isDisabled" /><span style='margin-left:20px'><b>通报人：</b></span><a-input v-model="inputPople1" :disabled="isDisabled" /></div> 
								</a-radio>
							</a-radio-group>
						</a-form-model-item>
					</div>
					<p><b>12.8<span class="star">*</span>开立异常事件调查单</b></p>
		...
		<a-modal
			...
			<div class="confirm-person">
				<div class="box"><span class="left">时间：</span><div class="right">
					<a-date-picker :valueFormat="valueFormat" style="width:100%" show-time @change="confirmPersonChange" v-model="confirmPersonObj.ppm" /></div></div>
				<div class="box" style="margin-bottom: 15px"><span class="left">GD种类</span><div class="right"><a-input allowClear v-model="confirmPersonObj.gdType" placeholder="请输入读值" /></div></div>
				<div class="box" style="margin-bottom: 15px"><span class="left">读值: ppm</span><div class="right"><a-input allowClear v-model="confirmPersonObj.readValue" placeholder="请输入读值" /></div></div>
			</div>
		</a-modal>
		...
		methods: {
			submitForm(formName) {
				this.$refs[formName].validate(valid => {
					if (valid) {
						let data = {
							commander: this.dynamicValidateForm.commander,
							ext: this.dynamicValidateForm.ext,
							reportTime: this.dynamicValidateForm.reportTime,
						...
						this.$http.post3('/chemInvolution/executeProcess',data).then((res) => {
							let data = {
			...
			saveData(){
				...
				this.$http.post3('/chemInvolution/saveOrUpdate',data).then((res) => {
				})
			},
			viewData(){
				this.$http.get3('/chemInvolution/queryOne?processInstanceId='+ this.processInstanceId).then((res) => {
					if (res.data.data){
					let tempData = res.data.data;
					if(tempData && tempData.id){
                        this.foukId = tempData.id
                    }
					this.dynamicValidateForm.commander = tempData.commander,
					this.dynamicValidateForm.ext = tempData.ext,
					this.dynamicValidateForm.reportTime = tempData.reportTime,
			...
			handleConfirmPerson(){
				if(this.confirmPersonObj.gdType){
					this.confirmList.push({
						gdType:this.confirmPersonObj.gdType,
						readValue: this.confirmPersonObj.readValue,
						ppm: this.confirmPersonObj.ppm
					})
