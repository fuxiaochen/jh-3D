<template>
	<div class="establishERT">
		...
		<template>
			...
			>
			<a-form-model-item>
				<p style="margin-left: -40px"><b>12.1*通报总经理成立全厂ERT</b></p>
				<div class="add-new-user">
					...
					<span style="margin-left: 20px;width:60px"><b>时间：</b></span><div class="right"><a-date-picker show-time v-model="confirmPersonObj.extensionTime"  :disabled="isDisabled"/> </div>
				</div>
			</a-form-model-item>

				<a-form-model-item prop="TVdate" >
					<div style="margin-left: -40px"><span><b>12.2*拨打119，通知消防队处理，时间：</b></span><a-date-picker show-time v-model="dynamicValidateForm.TVdate"  :disabled="isDisabled"/></div>
				</a-form-model-item>
				<a-form-model-item prop="TVdate" >
					<div style="margin-left: -40px"><span><b>12.3*通知保安到南门进行引导，并交通管制，时间：</b></span><a-date-picker show-time v-model="dynamicValidateForm.newDate" :disabled="isDisabled"/></div>
				</a-form-model-item>
				<a-form-model-item prop="TVdate" >
					<div style="margin-left: -40px"><span><b>12.4*消防队进厂时间：</b></span><a-date-picker show-time v-model="dynamicValidateForm.approachDate"  :disabled="isDisabled"/></div>
				</a-form-model-item>
				<a-form-model-item prop="TVdate" >
					<div style="margin-left: -40px"><span><b>12.5*与消防队进行汇报，简报时间：</b></span><a-date-picker show-time v-model="dynamicValidateForm.noticeDate " :disabled="isDisabled"/>
					<span style="margin-left: 20px"><b>文件交接时间：</b></span><a-date-picker show-time v-model="dynamicValidateForm.noticeDate1"  :disabled="isDisabled"/></div>
				</a-form-model-item>
				<a-form-model-item prop="TVdate" >
					<div style="display:flex">
						<div style="margin-left: -40px"><span><b>12.6*建立指挥中心，时间：</b></span><a-date-picker show-time v-model="dynamicValidateForm.establishDate"  :disabled="isDisabled"/></div>
				...
				<p style="margin-left: -40px"><b>12.7*任务分配</b></p>
				<div class="add-new-user">
				...
				<a-form-model-item prop="TVdate" >
					<div style="margin-left: -40px;display:flex"><span><b>12.8*灭火时间：</b></span><a-date-picker show-time v-model="dynamicValidateForm.endDate"  :disabled="isDisabled"/>
					<span style="margin-left: 20px"><b>消防队离厂时间：</b></span><a-date-picker show-time v-model="dynamicValidateForm.liveDate"  :disabled="isDisabled"/></div>
				</a-form-model-item>
				<a-form-model-item prop="TVdate" >
					<div style="margin-left: -40px;display:flex"><span><b>12.9*发送全厂ERT灭火成功简讯，时间：</b></span><a-date-picker show-time v-model="dynamicValidateForm.toDate"  :disabled="isDisabled"/>
		...
		methods: {
				...
				this.$refs[formName].validate(valid => {
					if (valid) {
						...
						let data = {
							...
							noticeTime: this.dynamicValidateForm.TVdate,
							controlTime: this.dynamicValidateForm.newDate,
							entryTime: this.dynamicValidateForm.approachDate,
							briefingTime: this.dynamicValidateForm.noticeDate,
							docHandoverTime: this.dynamicValidateForm.noticeDate1,
							commandTime: this.dynamicValidateForm.establishDate,
							location: this.local,
							assistanceSupport: popleList,
							envirMonitor: pople,
							fireFightTime: this.dynamicValidateForm.endDate,
							leaveTime: this.dynamicValidateForm.liveDate,
							newsSuccessFireTime: this.dynamicValidateForm.toDate,
							processInstanceId: this.processInstanceId,
						}
						this.$http.post3('/fire/factory/ert/excuteProcess',data).then((res) => {
			...
			saveData(){
				let pople = ''
						...
						this.$http.post3('/fire/factory/ert/saveOrUpdate',data).then((res) => {
							this.$message("success", "暂存成功");
						})
			},
			viewData(){
				this.$http.get3('/fire/factory/ert/queryOne?processInstanceId='+ this.processInstanceId).then((res) => {
					let tempData = res.data.data;
					// if(res.data.data){
                    //     let peopledata = "12.1总指挥官：:" + tempData.commander + ",时间：" + tempData.extensionTime + "\n" +
                    //     "12.6*建立指挥中心:" + ",时间：" + tempData.commandTime + ",位置：" + tempData.location + "\n"
                    //     "12.8*灭火时间:" + tempData.fireFightTime 
					...
					this.dynamicValidateForm.TVdate = tempData.noticeTime,
					this.dynamicValidateForm.newDate = tempData.controlTime,
					this.dynamicValidateForm.approachDate = tempData.entryTime,
					this.dynamicValidateForm.noticeDate = tempData.briefingTime,
					this.dynamicValidateForm.noticeDate1 = tempData.docHandoverTime,
					this.dynamicValidateForm.establishDate = tempData.commandTime,
					this.local= tempData.location
					...
					this.dynamicValidateForm.endDate = tempData.fireFightTime,
					this.dynamicValidateForm.liveDate = tempData.leaveTime,
					this.dynamicValidateForm.toDate = tempData.newsSuccessFireTime
                    let data = {
                        processInstanceId:this.processInstanceId
                    }
                    this.$http.get3('/fire/disposal/process/config/taskKeyList', data).then(res => {
