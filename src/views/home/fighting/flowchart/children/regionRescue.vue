    ...
    <div class="departmentRescue">
        ...
        <template>
            <a-form-model
                    ...
                    :rules="rules"
            >
                <p><b>11.1*管制组现场管制，并划分冷暖热区</b></p>
                <p class="padding-left-40">*现场环境持续侦测</p>
                ...
                <p class="padding-left-40">*根据现场环境情况时刻调整冷暖热区划分</p>
                <div class="rescueData padding-left-40">
                    <div class="add-new-user">
                ...
                <p><b>11.2*支援组支援器材</b></p>
                <div class="support-group">
                    <a-form-model-item prop="supportTime">
                        <div class="item">
                            <p><span>*器材支援到位时间：</span></p>
                            <a-date-picker style="width:200px" show-time v-model="rescueData.supportTime" @change="supportTimeChange" :disabled="isDisabled" />
                        </div>
                    </a-form-model-item>
                ...
                <p><b>11.3*抢救组进行着装</b></p>
                <div class="support-group">
                    <a-form-model-item prop="clothesTime">
                        <div class="item">
                            <p><span>*着装完成时间：</span></p>
                            <a-date-picker style="width:200px" show-time v-model="rescueData.clothesTime" @change="clothesTimeChange" :disabled="isDisabled" />
                        </div>
                    </a-form-model-item>
                ...
                <p><b>11.4*除污组架设除污站</b></p>
                <div class="support-group">
                    <a-form-model-item prop="cleanTime">
                        <div class="item">
                            <p><span>*除污站架设完成时间：</span></p>
                            <a-date-picker style="width:200px" v-model="rescueData.cleanTime" show-time @change="cleanTimeChange" :disabled="isDisabled" />
                        </div>
                    </a-form-model-item>
                ...
                <p><b>11.5*急救组架设急救</b></p>
                <div class="support-group">
                    <a-form-model-item prop="rescueTime">
                        <div class="item">
                            <p><span>*急救站架设完成时间：</span></p>
                            <a-date-picker style="width:200px" show-time v-model="rescueData.rescueTime" @change="rescueTimeChange" :disabled="isDisabled" />
                        </div>
                    </a-form-model-item>
                ...
                <div class="rescue-method">
                    ...
                    <div class="selectDetail">
                        <div class="selectItem">
                            <a-form-model-item prop="savePerson">
                                <a-checkbox class="checkBox"  v-model="rescueData.savePerson"  :disabled="isDisabled">
                                    救出受伤/被困人员
                                </a-checkbox>
                            </a-form-model-item>
                        </div>
                        <div class="selectValue" v-if="this.rescueData.savePerson">
                            <a-form-model-item label="时间" prop="saveTime">
                                <a-date-picker show-time v-model="rescueData.saveTime" @change="saveTimeChange" :disabled="isDisabled"/>
                            </a-form-model-item>
                        </div>
                    </div>
                    ...
                    <div class="selectDetail">
                        <div class="selectItem">
                            <a-form-model-item prop="closeGasSystem">
                                <a-checkbox class="checkBox"  v-model="rescueData.closeGasSystem"  :disabled="isDisabled">
                                    化学品/气体/燃气供应系统遮断
                                </a-checkbox>
                            </a-form-model-item>
                        </div>
                        <div class="selectValue" v-if="this.rescueData.closeGasSystem">
                            <a-form-model-item label="时间" prop="closeGasTime">
                                <a-date-picker show-time v-model="rescueData.closeGasTime" @change="closeGasTimeChange"  :disabled="isDisabled"/>
                            </a-form-model-item>
                        </div>
                    </div>
                    ...
                    <div class="selectDetail">
                        <div class="selectItem">
                            <a-form-model-item prop="rescueMeasure">
                                <a-checkbox class="checkBox"  v-model="rescueData.rescueMeasure"  :disabled="isDisabled">
                                    空调系统遮断
                                </a-checkbox>
                            </a-form-model-item>
                        </div>
                        <div class="selectValue" v-if="this.rescueData.rescueMeasure">
                            <a-form-model-item label="时间" prop="rescueMeasureTime">
                                <a-date-picker show-time v-model="rescueData.rescueMeasureTime" @change="rescueMeasureTimeChange" :disabled="isDisabled"/>
                            </a-form-model-item>
                        </div>
                    </div>
                    ...
                    <div class="selectDetail">
                        <div class="selectItem">
                            <a-form-model-item prop="closeEquipment">
                                <a-checkbox class="checkBox"  v-model="rescueData.closeEquipment" :disabled="isDisabled" >
                                    设备机台紧急遮断(断电/关闭供应阀门)
                                </a-checkbox>
                            </a-form-model-item>
                        </div>
                        <div class="selectValue" v-if="this.rescueData.closeEquipment">
                            <a-form-model-item label="时间" prop="closeEquipmentTime">
                                <a-date-picker show-time v-model="rescueData.closeEquipmentTime" @change="closeEquipmentTimeChange" :disabled="isDisabled"/>
                            </a-form-model-item>
                        </div>
                    </div>
                    ...
                    <div class="selectDetail">
                        <div class="selectItem">
                            <a-form-model-item prop="removeDangerGoods">
                                <a-checkbox class="checkBox"  v-model="rescueData.removeDangerGoods" :disabled="isDisabled">
                                    可燃性/危险性物品搬离
                                </a-checkbox>
                            </a-form-model-item>
                        </div>
                        <div class="selectValue" v-if="this.rescueData.removeDangerGoods">
                            <a-form-model-item label="时间" prop="removeTime">
                                <a-date-picker show-time v-model="rescueData.removeTime" @change="removeTimeChange"  :disabled="isDisabled"/>
                            </a-form-model-item>
                        </div>
                    </div>
                    <div class="selectDetail">
                        <div class="selectItem">
                            <a-form-model-item prop="outfire">
                                <a-checkbox class="checkBox"  v-model="rescueData.outfire"  :disabled="isDisabled">
                                    使用推车式CO2灭火器或架设水柱水雾灭火。
                                </a-checkbox>
                            </a-form-model-item>
                        </div>
                    </div>
                    <div class="selectDetail">
                        <div class="selectItem">
                            <a-form-model-item prop="waterFire">
                                <a-checkbox class="checkBox"  v-model="rescueData.waterFire"  :disabled="isDisabled">
                                    一组布水雾冷却暴露火场的钢瓶，直至火完全熄灭,一组布水线进行灭火。
                                </a-checkbox>
                            </a-form-model-item>
                        </div>
                    </div>
                    <div class="selectDetail">
                        <div class="selectItem">
                            <a-form-model-item prop="fireExtinguisher">
                                <a-checkbox class="checkBox"  v-model="rescueData.fireExtinguisher" :disabled="isDisabled" >
                                    使用推车式CO2灭火器或启动气体灭火系统灭火。
                                </a-checkbox>
                            </a-form-model-item>
                        </div>
                    </div>
                    <div class="selectDetail">
                        <div class="selectItem">
                            <a-form-model-item prop="kitchenFire">
                                <a-checkbox class="checkBox"  v-model="rescueData.kitchenFire"  :disabled="isDisabled">
                                    启动CMJS型厨房灭火设备。
                                </a-checkbox>
                            </a-form-model-item>
                        </div>
                ...
                <div class="success-failed">
                    ...
                    <a-form-model-item prop="fireSuccess">
                        <a-radio-group name="radioGroup" v-model="rescueData.fireSuccess" @change="stopFireChange" :disabled="isDisabled">
                            <a-radio :value="1">
                                灭火成功
                            </a-radio>
                            <a-radio :value="2">
                ...
                <div v-if="this.rescueData.fireSuccess === 1" class="stop-fire-succeed">
                    <a-form-model-item prop="fireSuccessTime">
                        <span>时间：</span>
                        <a-date-picker style="width:200px" v-model="rescueData.fireSuccessTime" show-time @change="fireSuccessTimeChange" :disabled="isDisabled"/>
                    </a-form-model-item>
                    <ul>
                        <li>
                            *灭火成功回报监控中心
                        ...
                        <li>
                            <a-form-model-item prop="exhaustTime">
                                *启动事故排烟，时间：<a-date-picker v-model="rescueData.exhaustTime" style="width:200px" show-time @change="exhaustTimeChange"  :disabled="isDisabled"/>
                            </a-form-model-item>
                        </li>
                        <li>
                            *抢救组撤至除污站进行除污，再到急救站进行健康评估
                        </li>
                        <li>
                            <a-form-model-item prop="newsFireSuccessTime">
                                *发送部门ERT灭火成功简讯，时间：<a-date-picker  v-model="rescueData.newsFireSuccessTime"  style="width:200px" show-time @change="newsFireSuccessTimeChange" :disabled="isDisabled"/>
                        ...
                        <li>
                            *除污组除污
                            <div class="padding-left-40 box-list">
                                <span>抢救组完成除污，</span>
                                <a-form-model-item prop="rescuePollutionTime">
                                    时间：<a-date-picker style="width:200px" v-model="rescueData.rescuePollutionTime" show-time @change="rescuePollutionTimeChange" :disabled="isDisabled" />
                                </a-form-model-item>
                                <a-form-model-item prop="rescuePollutionBar">
                                    记录栏：<a-input style="width:200px" v-model="rescueData.rescuePollutionBar" placeholder="记录" :disabled="isDisabled"/>
                                </a-form-model-item>
                            </div>
                            <div class="padding-left-40 box-list">
                                <span>现场完成除污，</span>
                                <a-form-model-item prop="scenePollutionTime">
                                    时间：<a-date-picker style="width:200px"  v-model="rescueData.scenePollutionTime" show-time @change="scenePollutionTimeChange" :disabled="isDisabled" />
                                </a-form-model-item>
                                <a-form-model-item prop="scenePollutionBar">
                                    记录栏：<a-input style="width:200px" v-model="rescueData.scenePollutionBar" placeholder="记录" :disabled="isDisabled"/>
                                </a-form-model-item>
                            </div>
                            <div class="padding-left-40 box-list">
                                <span>自身完成除污，</span>
                                <a-form-model-item prop="selfPollutionTime">
                                    时间：<a-date-picker style="width:200px" show-time v-model="rescueData.selfPollutionTime" @change="selfPollutionTimeChange" :disabled="isDisabled" />
                                </a-form-model-item>
                                <a-form-model-item prop="selfPollutionBar">
                                    记录栏：<a-input style="width:200px" v-model="rescueData.selfPollutionBar" placeholder="记录" :disabled="isDisabled"/>
                                </a-form-model-item>
                            </div>
                        </li>
                        <li>
                            *急救组健康评估
                            <div class="padding-left-40 box-list">
                                <span>抢救组完成健康评估，</span>
                                <a-form-model-item prop="healthyTime">
                                    时间：<a-date-picker style="width:200px" show-time v-model="rescueData.healthyTime" @change="healthyTimeChange"  :disabled="isDisabled"/>
                                </a-form-model-item>
                                <a-form-model-item prop="healthyBar">
                                    记录栏：<a-input style="width:200px" v-model="rescueData.healthyBar" placeholder="记录" :disabled="isDisabled"/>
                                </a-form-model-item>
                            </div>
                            <div class="padding-left-40 box-list">
                                <span>其余参与人员完成健康评估，</span>
                                <a-form-model-item prop="otherHealthyTime">
                                    时间：<a-date-picker style="width:200px" show-time v-model="rescueData.otherHealthyTime" @change="otherHealthyTimeChange"  :disabled="isDisabled"/>
                                </a-form-model-item>
                                <a-form-model-item prop="otherHealthyBar">
                                    记录栏：<a-input style="width:200px" v-model="rescueData.otherHealthyBar" placeholder="记录" :disabled="isDisabled"/>
                                </a-form-model-item>
                            </div>
                        </li>
                    </ul>
                ...
                <div v-if="this.rescueData.fireSuccess === 2" class="stop-fire-failed padding-left-40">
                    <div class="reason-time">

                        <a-form-model-item prop="fireFailureTime">
                            <span>时间：<a-date-picker style="width:200px" show-time v-model="rescueData.fireFailureTime" :disabled="isDisabled"/></span>
                        </a-form-model-item>
                        <a-form-model-item prop="fireFailureReason">
                            <span>原因：<a-input style="width:200px"  v-model="rescueData.fireFailureReason" placeholder="请输入原因" :disabled="isDisabled"/></span>
                        </a-form-model-item>
                        <span class="star">*</span>灭火失败回报监控中心
                        <a-form-model-item prop="newsFireSuccessTime">
                            <span class="star">*</span>发送部门ERT灭火失败简讯，时间：<a-date-picker  v-model="rescueData.newsFireFailureTime"  style="width:200px" show-time @change="newsFireSuccessTimeChange" :disabled="isDisabled"/>
                            <a href="http://nhoaaps01.chn.nexchip/AlarmSystem/Login.aspx#" title="发送简讯"  target="_blank" :disabled="isDisabled" style="padding-left:20px">发送简讯</a>
                        </a-form-model-item>
                    </div>
                </div>
        ...
        <a-modal
            ...
            <div class="confirm-person">
                <div class="box" style="margin-bottom: 15px"><span class="left">时间：</span>
                    <div class="right">
                        <a-date-picker style="width:100%" show-time v-model="environmentObj.ppm" @change="environmentChange"/>
                    </div>
                </div>
                <div class="box" style="margin-bottom: 15px"><span class="left">GD种类: </span>
                    <div class="right">
                        <a-input allowClear v-model="environmentObj.gdType" placeholder="请输入GD"/>
                    </div>
                </div>
        ...
        <a-modal
            ...
            <div class="confirm-person">
                <div class="box" style="margin-bottom: 15px"><span class="left">管制时间：</span>
                    <div class="right">
                        <a-date-picker style="width:100%" show-time v-model="coldWarmObj.controlTime" @change="coldWarmChange"/>
                    </div>
                </div>
        ...
        <a-modal
            ...
            <div class="rescue-group">
                ...
                <a-form-model-item label="进入时间">
                    <a-date-picker style="width:100%" show-time v-model="addRescueGroup.entryTime" @change="rescueGroupEnterChange" />
                </a-form-model-item>
                <a-form-model-item label="出来时间">
                    <a-date-picker style="width:100%" show-time v-model="addRescueGroup.leaveTime" @change="rescueGroupLeaveChange" />
                </a-form-model-item>
            </div>
        </a-modal>
        <a-modal
...
<script>
    ...
    export default {
        ...
        methods:{
            //回显测试
            viewData() {
                this.$http.get3('/fire/AreaErt/Rescue/queryOne?processInstanceId=' + this.processInstanceId).then(res => {
                    ...
                    this.rescueData = {
                        ...
                        cleanTime:tempData.decontaminationTime,
                        cleanRemark:tempData.decontaminationRecordBar,

                        rescueTime:tempData.emergencyTime,
                        rescueRemark:tempData.emergencyRecordBar,
                        //抢救措施：字段
                        savePerson:eval(tempData.savePerson),
                        saveTime:tempData.saveTime,

                        closeGasSystem:eval(tempData.closeGasSystem),
                        closeGasTime:tempData.closeGasTime,

                        rescueMeasure:eval(tempData.rescueMeasure),
                        rescueMeasureTime:tempData.rescueMeasureTime,


                        closeEquipment:eval(tempData.closeEquipment),
                        closeEquipmentTime:tempData.closeEquipmentTime,

                        removeDangerGoods:eval(tempData.removeDangerousGoods),
                        removeTime:tempData.removeTime,

                        outfire:eval(tempData.outfire),
                        waterFire:eval(tempData.outfire),
                        fireExtinguisher:eval(tempData.outfire),
                        kitchenFire:eval(tempData.outfire),

                        //灭火失败
                        fireSuccess:Number(tempData.fireSuccess),
                        fireFailureTime:tempData.fireFailureTime,
                        fireFailureReason:tempData.fireFailureReason,

                        //灭火成功
                        fireSuccessTime:tempData.fireSuccessTime,
                        exhaustTime:tempData.exhaustTime,
                        newsFireSuccessTime:tempData.newsFireSuccessTime,

                        rescuePollutionTime:tempData.rescueDecontaminationTime,
                        rescuePollutionBar:tempData.rescueDecontaminationBar,

                        scenePollutionTime:tempData.sceneDecontaminationTime,
                        scenePollutionBar:tempData.sceneDecontaminationBar,

                        selfPollutionTime:tempData.selfDecontaminationTime,
                        selfPollutionBar:tempData.selfDecontaminationBar,

                        healthyTime:tempData.healthyTime,
                        healthyBar:tempData.healthyBar,

                        otherHealthyTime:tempData.otherHealthyTime,
                        otherHealthyBar:tempData.otherHealthyBar,
                    }
                    ...
                    console.log('this.environmentList',this.environmentList,this.rescueData);
                    this.$http.get3('/fire/disposal/process/config/taskKeyList', data).then(res => {
                        for(let i of res.data.data){
                            if(i.isFinish == '1' && i.taskKey =='areaErtQj'){
                ...
                this.$refs[formName].validate(valid => {
                    if (valid) {
                        let data = {
                            ...
                            decontaminationTime:this.rescueData.cleanTime,
                            decontaminationRecordBar:this.rescueData.cleanRemark,
                            ...
                            removeDangerousGoods:this.rescueData.removeDangerGoods,
                            ...
                            rescueDecontaminationTime:this.rescueData.rescuePollutionTime,
                            rescueDecontaminationBar:this.rescueData.rescuePollutionBar,

                            sceneDecontaminationTime:this.rescueData.scenePollutionTime,
                            sceneDecontaminationBar:this.rescueData.scenePollutionBar,

                            selfDecontaminationTime:this.rescueData.selfPollutionTime,
                            selfDecontaminationBar:this.rescueData.selfPollutionBar,
                        ...
                        this.$http.post3('/fire/AreaErt/Rescue/excuteProcess', data).then(res => {
                            this.isDisabled = true
                            if (res.data.code === 1) {
                                let data = {
                                    processInstanceId:this.processInstanceId
            ...
            saveData(){
                ...
                this.$http.post3('/fire/AreaErt/Rescue/saveOrUpdate', data).then(res => {
                })
            },
            //添加现场环境持续检测
