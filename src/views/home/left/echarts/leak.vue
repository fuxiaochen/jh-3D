<template>
	<div id="leak" style="width:300px;height:148px"></div>
</template>

<script>
	import * as echarts from 'echarts';
	export default {
		name: "leak",
		methods:{
			initData2(data) {
				let myChart2 = echarts.init(document.getElementById("leak"));
				let seriesData = [
					{
						name: "正常",
						value: data.leakNormalStatistical,
					},
					// {
					// 	name: "故障",
					// 	value: data.leakNum2,
					// },
					// {
					// 	name: "预警",
					// 	value: data.leakNum6,
					// },
					{
						name: "报警",
						value: data.leakAlarmtatistical,
					},
					// {
					// 	name: "隔离",
					// 	value: data.leakNum5,
					// },
					// {
					// 	name: "离线",
					// 	value: data.leakNum7,
					// }
				];
				let option = {
					color:["#0492d4", "#ff8f40","#d34355","#36ecff","#ffffff","#a3a3a3"],
					tooltip: {
						trigger: 'item'
					},
					title: {
						text: "LEAK总数\n" + (data.leakNormalStatistical+data.leakAlarmtatistical),
						top: '47%',
						textAlign: "center",
						left: "33%",
						textStyle: {
							color: "#fff",
							fontSize: 14,
							fontWeight: 400
						},
					},
					legend: {
						type: "scroll",
						orient: "vertical",
						top: 50,
						left:180,
						selectedMode:false,
						textStyle: {
							color: "#fff",
							fontSize: 14,
							fontWeight: 100,
						},
						itemWidth: 6,
						itemHight: 21,
						data: [
							{
								name:'正常',
								icon:'image://data:image/png;base64,'
							},
							{
								name:'故障',
								icon:'image://data:image/png;base64,'
							},
							{
								name:'预警',
								icon:'image://data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAgAAAAUCAYAAACwG3xrAAAAfklEQVQoU2NkwALUX//n5Wdg8P3PxODHCJOHCTIwMoQy/GfwYGBk4GBgYPjBaPrmvykjA0M1w38Gd6ggsplgBRWMDAzt2KyCmTCqABKSQzocPjEwMGxA9wVIcDMjA8Pql58Zdj5QZAR7M4uBgcHy/3+G1R9EGHbeYWT8iZw2AMIpXyDOpZ/SAAAAAElFTkSuQmCC'
							},
							{
								name:'报警',
								icon:'image://data:image/png;base64,'
							},
							{
								name:'隔离',
								icon:'image://data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAgAAAAVCAYAAAB7R6/OAAAAbUlEQVQoU+3SvQkCYRCE4WcjfxCj60gQxEC0UL3IGgQjCzC1AzEc+bLj4BoQN9x9h4WZKROTZIFtDe9J1tjjiB1WlaTDASdsMBuKGvDGcupVAz6Y/4Ef9+GFfhz3ExeccauqNOCOa1tW1WPciy/NHFqwRCWYewAAAABJRU5ErkJggg=='
							},
							{
								name:'离线',
								icon:'image://data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAgAAAAVCAYAAAB7R6/OAAAAbElEQVQoke3SMQqDQBBG4S9mIUjAKkdKGm3iGbePIaews7G2DJ4i2BiQBa0tfDDNP29mmjnFGK2Q4xGSsECFGiWuATc857rjspwIGHFeu5NtNf/CJoewHyF9mCVffFJhwAsNWvwC+jl8o0vXTJtMDfM5eFdYAAAAAElFTkSuQmCC'
							},
						],
						formatter: function(name) { //图表名称+数值
							let pieLegendVale = {};
							seriesData.filter(item => {
								if (item.name === name) {
									pieLegendVale = item;
								}
							});
							return pieLegendVale.name + " " + pieLegendVale.value
						},
					},
					series: [
						{

							type: 'pie',
							radius: ['48%', '62%'],
							center: ['35%', '55%'],
							avoidLabelOverlap: false,
							label: {
								show: false,
								position: 'center'
							},
							itemStyle:{
								borderColor: '#030409',
								borderWidth: 2
							},
							// emphasis: {
							// 	label: {
							// 		show: true,
							// 		fontSize: '40',
							// 		fontWeight: 'bold'
							// 	}
							// },
							labelLine: {
								show: false
							},
							data:seriesData
						}
					]
				};
				myChart2.setOption(option);
			},
			getData(){
				this.$http.post2("/bigscreen/query/grid/gl").then(res => {
				  this.initData2(res.data.data.gridViewForLeakNum)
				});
			}
		},
		mounted() {
			this.getData()
		}
	}
</script>



